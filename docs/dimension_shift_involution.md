...
### 2.1 Recorded negative result: the complex slice is an affine map

The prototype formula `sigma(s) = 1 - conjugate(s)` is not itself a new
algebraic object. Writing `s` and `conjugate(s)` as independent formal
variables, the map is affine:

```text
sigma(s) = a*s + b*conjugate(s) + c     with  a = 0, b = -1, c = 1
```

Every such map is a similarity of the plane -- a composition of a rotation, a
scaling, and a translation -- so the prototype contributes no new algebra on
the complex slice. It is precisely the functional-equation reflection composed
with conjugation.

This is enforced by a test in the repository, not merely asserted here:
`python/riemann_framework/affine_reduction.py` classifies a candidate
expression by its centred second differences in the split coordinates
`u = (s + conjugate(s))/2`, `v = (s - conjugate(s))/2`, and
`python/tests/test_affine_reduction.py::test_dimension_shift_prototype_is_affine`
records the result. The check is exact for polynomial expressions.

What survives is the property the proposal actually needs, and it is verified
separately: the fixed locus of the prototype is exactly the critical line,
`sigma(s) = s  <=>  Re(s) = 1/2`
(`test_dimension_shift_prototype_fixed_locus_is_the_critical_line`).

The consequence for the programme is narrow but worth stating plainly. Any
attempt to build `K` as *new complex algebra* containing this formula is dead
on arrival -- the formula is already generated by the operations in `C`.
The real content of the proposal is therefore entirely in the remaining
requirements of section 3: an intrinsic structure whose complex slice happens
to be this reflection, together with a forcing theorem. A dimension shift that
is defined only by its action on the complex slice cannot be more than a
renamed reflection.
